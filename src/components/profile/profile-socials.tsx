            ...
            <div className="flex items-center gap-y-0 relative">
              ...
              <p
                onClick={() => {
                  console.log("test");
                  toast(
                    "You are in edit mode! Click again to go back to preview mode.",
                    { position: "top-center" },
                  );
                  setPreview(!preview);
                }}
                className={cn(
                  "flex items-center cursor-pointer gap-x-1 text-white text-xs bottom-0 right-0 underline opacity-80 hover:text-card-bg-light absolute",
                  color === "light" ? "text-black" : "text-white",
                )}
              >
                <Pencil size="14" className="mt-[.08rem]" />
                Edit Mode
              </p>
            </div>
            {items &&
              items.map((social, index) => (
                <Link
                  key={social.id}
                  className={cn(
                    "flex items-center bg-slate-600/50 text-white w-full p-2 z-50 cursor-pointer rounded-md",
          ...
          <div className="flex items-center gap-y-0 relative">
            ...
            <p
              onClick={() => {
                console.log("test");
                toast(
                  "You are in preview mode! Click again to go back to edit mode.",
                  { position: "top-center" },
                );
                setPreview(!preview);
              }}
              className={cn(
                "flex items-center gap-x-1 text-white text-xs bottom-0 right-0 underline opacity-80 hover:text-card-bg-light absolute cursor-pointer",
                color === "light" ? "text-black" : "text-white",
              )}
            >
              <Eye size={16} className="mt-[.12rem]" />
              Preview Profile
            </p>
          </div>
