        ...
        {items && items.length > 0 &&
          <div className="flex flex-col gap-y-2 z-50" >
            <div className='flex items-center gap-y-0 relative'>
              ...
              <p onClick={() => { console.log('test'); toast('You are in edit mode! Click again to go back to preview mode.', { position: 'top-center' }); setPreview(!preview) }} className={cn('cursor-pointer text-white text-xs bottom-0 right-0 underline opacity-80 hover:text-card-bg-light absolute', color === 'light' ? 'text-black' : 'text-white')}>Edit Mode</p>
            </div>
    ...
    <section className='w-full mt-6 z-50 mb-20'>
      {items && items.length > 0 &&
        <Reorder.Group as="div" className="flex flex-col gap-y-2 z-50" axis="y" values={items} onReorder={handleUpdateItems}>
          <div className='flex items-center gap-y-0 relative'>
            <h1 className={cn(`text-xl`, color === 'light' ? 'text-black' : 'text-white')}>Socials</h1>
            <p onClick={() => { console.log('test'); toast('You are in preview mode! Click again to go back to edit mode.', { position: 'top-center' }); setPreview(!preview) }} className={cn('text-white text-xs bottom-0 right-0 underline opacity-80 hover:text-card-bg-light absolute cursor-pointer', color === 'light' ? 'text-black' : 'text-white')}>Preview Profile</p>
          </div>
